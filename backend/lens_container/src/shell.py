...
class CommandShell(object):
    ...
    def __init__(self, root, driver):
        ...
        self.root = root
        self.layer_classes = base_layer.LayerMeta.layer_classes
        self.input_buffer = ""
        ...
        self.write_prompt()
        base_layer.LayerMeta.instance_callback = self.instance_callback
    ...
    def do_load(self, graph_file):
        ...
        v = {"root": self.root}
        exec(open(graph_file).read(),v)
        print("Loaded '{0}'".format(graph_file))

    def do_driver(self, mode):
        """Manually change the tap board mode"""
        if mode == "active":
            self.driver.active()
        elif mode == "passive":
            self.driver.passive()
